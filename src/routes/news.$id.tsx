...
export const Route = createFileRoute("/news/$id")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: loaderData ? `${loaderData.item.title} — NIGP` : "خبر" },
      { name: "description", content: loaderData?.item.excerpt ?? "خبر حزب" },
    ],
  }),
  component: NewsDetail,
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-4 py-16 text-center">
      <h1 className="text-2xl font-black">خبر یافت نشد</h1>
      <Link to="/news" className="mt-4 inline-block text-brand hover:underline">
        بازگشت به اخبار
      </Link>
    </div>
  ),
...
type Comment = { id: number; author: string; text: string; replies: Comment[] };
...
function NewsDetail() {
  ...
  function addComment() {
    if (!text.trim()) return;
    const c: Comment = { id: Date.now(), author: "کاربر مهمان", text: text.trim(), replies: [] };
    if (replyTo == null) {
      setComments((cs) => [c, ...cs]);
    } else {
      setComments((cs) =>
        cs.map((x) => (x.id === replyTo ? { ...x, replies: [...x.replies, c] } : x)),
      );
    }
    setText("");
    setReplyTo(null);
    ...
    <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      <section className="mt-12 border-t border-border pt-8">
        <div className="mb-6 flex items-center gap-2">
          ...
          <h2 className="text-xl font-black text-ink">نظرات</h2>
        </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4 shadow-card">
          {replyTo != null && (
            <div className="mb-2 flex items-center justify-between rounded bg-muted px-3 py-2 text-xs">
              ...
              <button type="button" onClick={() => setReplyTo(null)} className="text-brand">
                انصراف
              </button>
            </div>
          )}
        ...
        <div className="mt-6 space-y-4">
          ...
          {comments.map((c) => (
            <CommentView key={c.id} c={c} onReply={() => setReplyTo(c.id)} />
          ))}
        </div>
      </section>
    </article>
...
function CommentView({ c, onReply }: { c: Comment; onReply: () => void }) {
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-center justify-between text-xs">
        <span className="font-bold text-ink">{c.author}</span>
        <button type="button" onClick={onReply} className="font-bold text-brand hover:underline">
          پاسخ
        </button>
      </div>
        ...
        <div className="mr-4 mt-4 space-y-3 border-r-2 border-brand/30 pr-4">
          {c.replies.map((r) => (
            <div key={r.id} className="rounded-md bg-muted/60 p-3">
              <div className="text-xs font-bold text-ink">{r.author}</div>
